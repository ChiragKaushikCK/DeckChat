...
def text_to_speech(text):
    """Convert text to speech using gTTS (female AI voice by default for English)"""
    try:
        # Generate speech
        ...
        tts.write_to_fp(fp)
        fp.seek(0)
        # Convert to base64 for playback and database storage
...
# Database Functions
# ----------------------
def save_message(user_email: str, role: str, content: str, model_used: str = None, audio_base64: str = None):
    """Save message to Firebase with metadata and optional audio data"""
    if db:
        try:
            message_data = {
                ...
                'audio_base64': audio_base64  # Store Base64 audio directly in DB
            }
    ...
    try:
        ...
        for doc in docs:
            ...
            messages.append({
                ...
                'audio_base64': data.get('audio_base64', None) # Fetch audio history
            })
...
def export_chat_history(messages: List[Dict], format: str = "txt"):
    ...
    if format == "txt":
        ...
        # Remove audio base64 from json export to keep it lightweight
        export_msgs = [{k:v for k,v in m.items() if k != 'audio_base64'} for m in messages]
        return json.dumps(export_msgs, indent=2, default=str)
...
def show_chat_interface():
    ...
    with st.sidebar:
        ...
        model_option = st.radio(
            ...
            help="Base: GPT-3.5 | Pro: Llama-3-70B"
        )
    ...
    # Display chat messages with Audio Playback
    for msg in st.session_state.messages:
        with st.chat_message(
            ...
            # Show audio player if audio base64 is present
            if msg.get('audio_base64'):
                audio_format = "audio/mp3" if msg['role'] == 'assistant' else "audio/wav"
                st.audio(base64.b64decode(msg['audio_base64']), format=audio_format)
    ...
    # Handling Input Forms (Text or Audio)
    audio_prompt = st.audio_input("🎤 Record Voice Message")
    ...
    if text_prompt:
        ...
        # Check if we've already processed this audio payload to prevent re-triggering loops
        ...
        if st.session_state.get('last_processed_audio') != audio_hash:
            ...
            with st.spinner("🎙️ Transcribing voice..."):
                ...
                if "Audio recognition failed" not in transcribed:
                    final_prompt = transcribed
                    user_audio_b64 = base64.b64encode(audio_bytes).decode()
                else:
    ...
    if final_prompt:
        ...
        with st.chat_message("user", avatar="🧑"):
            ...
            if user_audio_b64:
                st.audio(base64.b64decode(user_audio_b64), format="audio/wav")
        
        # Add to session
        st.session_state.messages.append({
            "role": "user",
            "content": final_prompt,
            "audio_base64": user_audio_b64
        })
        
        # Save to database (Includes base64 audio string to store in Firestore)
        save_message(
        ...
        # Add current prompt
        messages_for_model.append(HumanMessage(content=final_prompt))
        ...
        with st.chat_message("assistant", avatar="✨"):
            ...
            try:
                ...
                with st.spinner(""):
                    ...
                    # Generate Audio Response (Female AI Voice)
                    with st.spinner("🔊 Generating voice..."):
                        assistant_audio_b64 = text_to_speech(full_response)
                        if assistant_audio_b64:
                            # Auto-play the AI voice response
                            st.audio(base64.b64decode(assistant_audio_b64), format="audio/mp3", autoplay=True)
